...
2. Внешний consumer: `{ "extends": ["@omnifield/biome-config/biome.json"] }` (через npm)
...
- **Config-only пакет — нет `src/`, нет `dist/`.** `package.json.files: ["biome.json"]`, `exports: { "./biome.json": "./biome.json" }`. `dev:builders` в root исключает пакет флагом `--filter "!@omnifield/biome-config"` — у него нет `build`/`dev` скриптов. Это норма, не баг.
...
- `@omnifield/biome-config` **НЕ входит** в группу `cli`. Версионируется независимо (own version 0.0.10).
